...
class ResNet(nn.Module):
    def __init__(
        ...
        print(resnet_v, f'with dropout = {args.resnet_dropout}')
        if resnet_v == 'resnet18':
            self.resnet = resnet18(pretrained=False, dropout=self.args.resnet_dropout)
        elif resnet_v == 'resnet50':
            self.resnet = resnet50(pretrained=False, dropout=self.args.resnet_dropout)
        elif resnet_v == 'resnet101':
            self.resnet = resnet101(pretrained=False, dropout=self.args.resnet_dropout)
        else:
