...
class UNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if init_shape[2] < 400 and False:
            ...
            up = Resize(spatial_size=[400, 400, 128])
            x_f = x.reshape(-1, 100, 100, 32).cpu().detach().numpy()
            x = up(x_f).reshape(init_shape[0], init_shape[1], 400, 400, 128)
            x = torch.Tensor(x).to(self.device)
        '''
        x = self.model(x)
        '''
        if init_shape[2] < 400 and False:
            down = Resize(spatial_size=[100, 100, 32])
            x_f = x.reshape(-1, 400, 400, 128).cpu().detach().numpy()
            x = down(x_f)
            x = x.reshape(init_shape[0], 2 * 1, 100, 100, 32) # assume init_shape[1] == 1 TODO add argument about mask_attention to avoid hard-coded shapes
            x = torch.Tensor(x).to(self.device)
        '''
        return x
